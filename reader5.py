...
if 'pub' in ROOT_PROJ_PATH:
    ...
    LABEL_IDS = {"pants-fire": 0, "false": 1, "barely-true": 2, "half-true": 3, "mostly-true": 4, "true": 5}
elif 'gossipcop' in ROOT_PROJ_PATH:
...
class myDataset(Dataset):
    ...
    def read_from_dir(self, path):
        ...
        if os.path.isfile(path) and ('LIAR-RAW' in path or "gossipcop" in path):
    ...
    def load_raw_gossip(self, df):
        '''parsing dict objs to list '''
        raw_data = [[] for _ in range(9)]  # event_id, claim, label, explain, (link, content, domain, report_sents, report_is_evidence) 0/1
        for obj in tqdm(df):  # 遍历字典的值
            #print(obj)
            report_tok_sents = []
            report_tok_sent_ids = []
            ...
            event_id = obj['id']
            claim = obj.get('title', obj['text'][:50])  # 使用标题或文本前50个字符作为claim
            label = obj['label']
            explain = obj['text'].split('\n')[0]  # 使用新闻文本的第一段作为解释

            reports = [{
                "link": obj['url'],
                "content": obj['text'],
                "domain": obj['source'],
                "tokenized": [{"sent": sent, "is_evidence": 0} for sent in obj['text'].split('. ') if sent]
            }]
