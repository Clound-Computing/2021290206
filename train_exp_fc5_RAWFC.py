...
from model_exp_fc5_ablation_RS import ExplainFC #DistilEmbeddings

# from dataset.oracles.vocab_process import preprocess
from helpers.path_util import from_project_root, dirname

# from eval import evaluate_model
from eval_exp_fc5_rawfc import evaluate_model, loss_func
...
ROOT_PROJ_PATH = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/Codes/dataset/RAWFC/"
...
LOG_FILE = pjoin(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/Codes/dataset/logs", f"{date_str}_DOC_ExplainFC5-DistilBERT_auto_{ROOT_PROJ_PATH.split('/')[-1]}2-all.log")
...
def train_model(n_epochs=n_epochs,
                train_file = "train",##containing all data
                val_file = "val",
                test_file = 'test',
    ...
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=dataset.my_collate, num_workers=4, pin_memory=True)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (oracle_ids, label_ids, raw_text_dict, lm_ids_dict) in enumerate(train_loader, 1):
            ...
            cla_loss = loss_func(criterion, pred_veraciy.to(device), label_ids.to(device))
            ...
            for pred_evi,oracle,masks in zip(pred_evi_logits, gold_ids, selected_sent_repr_mask):
                evi_loss.append(loss_func(sent_criterion, pred_evi.to(device), oracle.float().to(device), mask=masks))
            ...
            accuracy = (pred.cpu() == label_ids.cpu()).float().mean()
            ...
            evi_accuracy = mean([torch.mean((pids.to(device) == gids.to(device)).float()).item() for pids,gids in zip(pred_ids, gold_ids)])
