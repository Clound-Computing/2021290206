...
#             self.article_source_embedding = nn.Embedding(len(self.article_source_dict), self.source_dim, padding_idx=0)
    ...
    def forward(self, oracle_ids, labels=None, lm_ids_dict=None):
        ...
        for i, (pad_c_repr, pad_s_repr, real_num_src) in enumerate(zip(claim_sent_repr, src_sent_repr, src_sent_num)):
            ...
            s_unpacked = torch.max(s_unpacked, dim=1)[0]
            doc_scores = torch.matmul(torch.matmul(s_unpacked, self.params['doc_att_w']), pad_c_repr.squeeze(0))
            sorted_score, indices = torch.sort(doc_scores, descending=True)
            _indices = indices.tolist()[:TOP_N_DOC]
            selected_indices_list.append(_indices)
            sel_s_repr = pad_sequence([pad_s_repr[i] for i in _indices], batch_first=True)
            sel_s_mask = pad_sequence([_mask[i] for i in _indices], batch_first=True)
            ...
            for doc_repr, s_repr, s_mask in zip(s_unpacked, sel_s_repr, sel_s_mask):
                ...
                s_mask = 1 - s_mask
                # 1) claim-relevance, topic
                # claim_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask == 1, -1e9), self.params['doc_sent_att_w']), pad_c_repr.squeeze(0)) / math.sqrt(s_repr.shape[-1])  # 移除相关性
                # 2) entire doc-relevance, sailence
                # doc_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask == 1, -1e9), self.params['doc_sent_att_w2']), doc_repr) / math.sqrt(s_repr.shape[-1])  # 移除丰富性
                # 3) content Wh ; doc_sent_att_w3
                content_scores = self.content_layer(s_repr.masked_fill(s_mask == 1, -1e9)).squeeze() / math.sqrt(s_repr.shape[-1])
                # 4) novelty
                # red_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask == 1, -1e9), self.params['doc_sent_att_w3']), h_reduncy) / math.sqrt(s_repr.shape[-1])  # 移除非冗余
                # total_weights = claim_scores + doc_scores + content_scores - red_scores  # 所有特征
                # 如果单独移除相关性
                total_weights_relevance = doc_scores + content_scores - red_scores
                # 如果单独移除丰富性
                total_weights_richness = claim_scores + content_scores - red_scores
                # 如果单独移除显著性
                total_weights_salience = claim_scores + doc_scores + content_scores
                # 如果单独移除非冗余
                total_weights_non_redundancy = claim_scores + doc_scores + content_scores
                pre_prob = self.sigmoid(total_weights)
                evi_logits[i].append(pre_prob)
                h_reduncy = self.tanh(torch.sum(s_repr * pre_prob.unsqueeze(-1), dim=0))
                ind_list = []
