...
#             'doc_sent_att_b': nn.Parameter(torch.FloatTensor(self.n_hidden).uniform_(0, 0.2) , requires_grad=True),
   ...
   def forward(self, oracle_ids, labels=None, lm_ids_dict=None):
        ...
        claim_sent_repr = claim_repr
        src_sent_repr = [pad_sequence(src, batch_first=True) for src in src_repr]
        src_sent_num = lm_ids_dict['src_sent_num']
        ...
        for i, (pad_c_repr, pad_s_repr, real_num_src) in enumerate(zip(claim_sent_repr, src_sent_repr, src_sent_num)):
            ...
            s_unpacked = torch.max(s_unpacked, dim=1)[0]
            doc_scores = torch.matmul(torch.matmul(s_unpacked, self.params['doc_att_w']), pad_c_repr.squeeze(0))  # 移除 RS
            sorted_score, indices = torch.sort(doc_scores, descending=True)  # 移除 RS
            _indices = indices.tolist()[:TOP_N_DOC]  # 移除 RS
            selected_indices_list.append(_indices)  # 移除 RS
            sel_s_repr = pad_sequence([pad_s_repr[i] for i in _indices], batch_first=True)  # 移除 RS
            sel_s_mask = pad_sequence([_mask[i] for i in _indices], batch_first=True)  # 移除 RS
            selected_sent_repr_mask.append(sel_s_mask)
            #h_reduncy = torch.zeros(sel_s_repr.shape[-1]).to(self.device)  # 移除 SE
            for doc_repr, s_repr, s_mask in zip(s_unpacked, sel_s_repr, sel_s_mask):
                threshhold = 1.0/sum(s_mask) if sum(s_mask) > 1 else 1.0/2
                batch_sel_thresholds[i].append(threshhold)
                s_mask = 1 - s_mask
                claim_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask.unsqueeze(1) == 1, -1e9), self.params['doc_sent_att_w']), pad_c_repr.squeeze(0)) / math.sqrt(s_repr.shape[-1])
                doc_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask.unsqueeze(1) == 1, -1e9), self.params['doc_sent_att_w2']), doc_repr) / math.sqrt(s_repr.shape[-1])
                content_scores = self.content_layer(s_repr.masked_fill(s_mask.unsqueeze(1) == 1, -1e9)).squeeze() / math.sqrt(s_repr.shape[-1])
                # red_scores = torch.matmul(torch.matmul(s_repr.masked_fill(s_mask == 1, -1e9), self.params['doc_sent_att_w3']), h_reduncy) / math.sqrt(s_repr.shape[-1])  # 移除 SE
                total_weights = claim_scores + doc_scores + content_scores  # - red_scores  # 移除 SE
                pre_prob = self.sigmoid(total_weights)
                evi_logits[i].append(pre_prob)
                # h_reduncy = self.tanh(torch.sum(s_repr * pre_prob.unsqueeze(-1), dim=0))  # 移除 SE
                ind_list = []
